package com.zhangliangming.hp.ui.db;
...
import com.zhangliangming.hp.ui.util.PingYinUtil;
...
public class SongDB {
     ...
     * @return
     */
    public List<AudioInfo> getAllLocalCategorySong(String category) {
     ...
     * @return
     */
    public List<AudioInfo> getAllLikeSong() {
        List<AudioInfo> list = new ArrayList<AudioInfo>();
        db = mDBHlper.getReadableDatabase();
     ...
     * @return
     */
    public List<AudioInfo> getDownloadSong(int status) {
        List<AudioInfo> list = new ArrayList<AudioInfo>();
        db = mDBHlper.getReadableDatabase();
        Cursor cursor = db.query(TBL_NAME, null,
     ...
     * @return
     */
    public List<AudioInfo> getAllLocalSong() {
        List<AudioInfo> list = new ArrayList<AudioInfo>();
        db = mDBHlper.getReadableDatabase();
     ...
     * 获取所有的推荐歌曲
     *
     * @param isInit 是否是初始化，app启动时，先把没下载完成的歌曲删除
     * @return
     */
    public List<AudioInfo> getAllRecommendSong(boolean isInit) {
        List<AudioInfo> list = new ArrayList<AudioInfo>();
        db = mDBHlper.getReadableDatabase();
     ...
     * @return
     */
    public boolean songIsExists(String filePath) {
        db = mDBHlper.getReadableDatabase();
        // 第一个参数String：表名
        // 第二个参数String[]:要查询的列名
        // 第三个参数String：查询条件
        ...
        // 第六个参数String：对分组的结果进行限制
        // 第七个参数String：对查询的结果进行排序
        Cursor cursor = db.query(TBL_NAME, new String[]{"filePath"},
                " filePath=?", new String[]{filePath}, null, null, null);
        if (!cursor.moveToNext()) {
            cursor.close();
            return false;
        }
